...
interface Props {
  studentList: DetailedMember[]
  course: Course
}

export default function AdminScoreStatistic({ studentList, course }: Props) {
  //! Calculate score
  const attendanceRatio = course.attendance_ratio as number
  const labRatio = course.lab_ratio as number
  const midtermRatio = course.midterm_ratio as number
  const finalRatio = course.final_ratio as number
